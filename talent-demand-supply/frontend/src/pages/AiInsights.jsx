...
const calculateHeuristicScore = (employee, targetSkill) => {
  ...
  const hasSkill = employee.skills.some(
    (s) => s.toLowerCase().includes(targetSkill.toLowerCase())
  );
  ...
  const getRecommendations = async () => {
    ...
    try {
      ...
      const analyzedData = res.data
        .filter(emp => emp.skills.some(s => s.toLowerCase().includes(skill.toLowerCase())))
        .map(emp => ({
          ...emp,
    ...
    <div style={{ padding: "40px", backgroundColor: "#f8fafc", minHeight: "100vh" }}>
      <div style={{ maxWidth: "900px", margin: "0 auto" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: "40px" }}>
          ...
          <h1 style={{ fontSize: "32px", fontWeight: "800", color: "#1e293b" }}>AI Talent Matcher</h1>
          <p style={{ color: "#64748b" }}>Solving the Experience vs. Expertise gap using Heuristic Scoring.</p>
        </div>
            ...
            <div key={i} style={styles.resultCard}>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <h3 style={{ margin: 0, fontSize: '18px', color: '#1e293b' }}>{emp.name}</h3>
                  {i === 0 && (
                    <span style={styles.badge}><Award size={12} /> Top Match</span>
                  )}
                </div>
                <p style={{ margin: "5px 0", color: "#64748b", fontSize: "14px" }}>
                  <strong>Dept:</strong> {emp.department} • <strong>Exp:</strong> {emp.experience} Years
                </p>
              ...
              <div style={{ textAlign: "right", minWidth: "180px" }}>
                <div style={{ color: emp.fitmentScore > 75 ? "#10b981" : "#2563eb", fontWeight: "800", fontSize: "28px" }}>
                  {emp.fitmentScore}%
                </div>
                <div style={styles.progressContainer}>
                  <div style={{ 
                    ...
                    backgroundColor: emp.fitmentScore > 75 ? "#10b981" : "#3b82f6" 
                  }}></div>
                </div>
                <div style={{ fontSize: "11px", color: "#94a3b8", fontWeight: "700", textTransform: "uppercase" }}>Fitment Probability</div>
              </div>
            </div>
          ))}
...
const styles = {
  searchContainer: { 
    ...
    padding: "10px 20px", borderRadius: "15px", border: "1px solid #e2e8f0", 
    boxShadow: "0 4px 6px rgba(0,0,0,0.02)", marginBottom: "40px" 
  },
  input: { flex: 1, border: "none", outline: "none", fontSize: "16px", padding: "10px" },
  button: { 
    backgroundColor: "#0b3d91", color: "white", padding: "12px 25px", 
  ...
  badge: { 
    backgroundColor: "#fef3c7", color: "#d97706", padding: "2px 8px", 
    borderRadius: "12px", fontSize: "11px", fontWeight: "800", display: "flex", alignItems: "center", gap: "4px" 
  },
  skillTag: { backgroundColor: "#f1f5f9", color: "#475569", padding: "2px 8px", borderRadius: "4px", fontSize: "12px" },
  progressContainer: { width: "100%", height: "8px", backgroundColor: "#f1f5f9", borderRadius: "10px", margin: "8px 0" },
  progressFill: { height: "100%", borderRadius: "10px", transition: "width 0.5s ease-in-out" }
};
